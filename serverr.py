...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            ...
            response = "Please specify a user ID to add 😵‍💫🆔️ ."
    else:
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"🤖tara user ID: {user_id}"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 30:
                response = "You Are On Cooldown . Please Wait 30sec Before Running The /bgmi Command Again."
                bot.reply_to(message, response)
                return
...
def welcome_start(message):
    ...
    response = f'''WELCOME TO ABHISHEK, {user_name}! FOR BUY DDOS .Dm: @BABBY_HACK2
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name},👑OWNER Command👑:
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "⚠️ Message To All Users By OWNER:\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
